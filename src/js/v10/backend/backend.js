...
var bindEvents = function() {
	...
	addCloneButton();
   	...
   	$('body').on('click', '.js-module-clone', function(e) {
        ...
        var $item = $clone.insertAfter($(this).parent().parent());
...
var compileCSS = function(newLess) {
	...
	var whiteLabelURL = 'http://nd-auto-styles-temp-production.s3.amazonaws.com/01.1%20Core%20Mobile';
...
// ==UserScript==
// @name            Site Editor Extended
// @namespace       http://www.parsonsprojects.co.uk/
// @version         2.9
// @description     Site editor add predefined modules
// @include         *
// @exclude     	*/backend
// @copyright       2012+, Alan Parsons
// @require         https://ajax.googleapis.com/ajax/libs/jquery/2.1.3/jquery.min.js
// @require         https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.0/jquery-ui.min.js
// @require         https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/2.0.0/bootstrap.min.js
// @icon            http://www.parsonsprojects.co.uk/wp-content/themes/parsonsprojects/img/avatar.png
// @grant       	GM_xmlhttpRequest
// @connect         css.blenheim.gforcestestlink.co.uk
// @connect         cdn.jsdelivr.net
// ==/UserScript==

// !TODO
// move wrap modules into function
// simplify removing a module - admin
// template needs to be saved for new module
// error messaging/less/save error
// ability to upload image over screenshot - need to add database entry
// load default tags
// add copy button for js
...
setTimeout(function(){ codeBlocks.init(); }, 500);
...
var frontendButtons = function() {
	...
	function checkDeploy() {
		...
		var checkDeployTimeout = setTimeout(function() {

			$.ajax({
	            url: 'http://' + window.location.hostname + '/backend/assets/',
	            type: 'GET'
	        }).done(function(response) {

			    if(response) {
			    	var html = $($.parseHTML(response));
			    	if(html.find('#Site_assets_path option:nth-child(2)').is(':selected')) {
	                	location.reload();
			    	} else {
			    		checkDeploy();
			    	}
		...
		return new Promise(function(resolve) {
			...
			$.ajax({
			    ...
			    if(response) {
	                ...
	                if(error.length) loggedIn = false
	                else loggedIn = true;
	            }
	            resolve();
...
var loadModules = function(data) {
    ...
    data.forEach(function(item) {
    	if(item) {
    		...
    		var img_url = 'http://css.blenheim.gforcestestlink.co.uk/wp-content/themes/css-wiki/template-editor/preview/' + item.id + '.png';
...
var renderMarkup = function() {
    ...
    newButtons = 	'<li class="nd-divider"></li><li><a href="" title="Insert Template" class="js-insert-template">Insert Template</a></li>' +
   	...
   	orderModal = '<div class="order-modal-overlay"></div><div class="order-modal"><a href="#" class="save">Save</a><a href="#" class="cancel">Cancel</a></div>';
    ...
    plugin.orderModal = $(orderModal).appendTo('#nd-modals');
